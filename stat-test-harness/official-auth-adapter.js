...
(function (global) {
  const DEFAULT_CONFIG = Object.freeze({
    ...
    probe: { intervalMs: 2500, maxAttempts: 24 },
  ...
  function normalizeConfig(input = {}) {
    return {
      ...DEFAULT_CONFIG,
      ...input,
      probe: { ...DEFAULT_CONFIG.probe, ...(input.probe || {}) }
    };
  }
  ...
  async function readSession(config) {
    ...
    if(!url)return {supported:false,code:"NOT_CONFIGURED",reason:"No documented official session endpoint configured"};
  ...
  async function readStats(config) {
    ...
    if(!url)return {supported:false,code:"NOT_CONFIGURED",reason:"No documented official player-stat endpoint configured"};
    const result=await fetchJson(url,config.requestTimeoutMs);
    if(!result.ok)return {supported:true,...result};
  ...
  async function setModifiers(config, modifiers) {
    ...
    if(!url)return {supported:false,code:"NOT_CONFIGURED",reason:"No documented official modifier endpoint configured"};
    const result=await fetchJson(url,config.requestTimeoutMs,{method:"POST",body:{modifiers}});
    if(!result.ok)return {supported:true,...result};
    return {
      supported:true,ok:true,
  ...
  async function restore(config) {
    ...
    if(!url)return {supported:false,code:"NOT_CONFIGURED",reason:"No documented official restoration endpoint configured"};
    const result=await fetchJson(url,config.requestTimeoutMs,{method:"POST",body:{}});
    if(!result.ok)return {supported:true,...result};
    return {supported:true,ok:true,stats:result.data?.stats||null};
  }

  const api={
    DEFAULT_CONFIG,normalizeConfig,validateOfficialEndpoint,
    readSession,readStats,setModifiers,restore
  };
