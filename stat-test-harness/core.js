...
(function (global) {
  ...
  const READ_CAPABILITIES = Object.freeze(["read-player-stats"]);
  const WRITE_CAPABILITIES = Object.freeze(["apply-test-modifiers", "restore-player-stats"]);
  ...
  function normalizeFlags(input) {
    const out = emptyFlags(false);
    if (!input || typeof input !== "object") return out;
    for (const def of STAT_DEFS) out[def.key] = Boolean(input[def.key]);
    return out;
  }
  ...
  function normalizeStats(stats) {
    if (!validateStats(stats)) return null;
    return Object.fromEntries(STAT_DEFS.map(s => [s.key, Number(stats[s.key])]));
  }
  ...
  function detect(baseline, measured, applied) {
    let enabledCount = 0, mismatchCount = 0, signatureMatches = 0;
    for (const stat of STAT_DEFS) {
      const base = Number(baseline[stat.key]);
  ...
  const api = {
    ...
    emptyFlags, normalizeFlags, calculateEffective,
    validateStats, normalizeStats,
